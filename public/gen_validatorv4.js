...
function TestPan(objValue, strError)
{
    var ret = true;
    if (objValue.value.length > 0 && !pan_validate(objValue.value))
    {
        if (!strError || strError.length == 0)
        {
            strError = objValue.name + ": Enter a valid PAN No ";
        } //if                                               
        sfm_show_error_msg(strError, objValue);
        ret = false;
    } //if 
